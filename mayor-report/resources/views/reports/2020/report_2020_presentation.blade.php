...
@section('css_import')
    @parent
    <link rel="stylesheet" href="{{ asset('/css/report_presentation_view.css') }}">
@endsection
...
@php
    $presentation = [
        [
            ...
            'content' => '                <div style="                    width: 100%;                    height: 5.5%;                    background-color: red;                    position: absolute;                    top: 2%;                " class="shadow">                    <a style="color: white; position: absolute; text-decoration: none; font-weight: 700; top: 50%; left: 85%; transform: translate(-50%, -50%); width: fit-content; font-size: 12.76px;" fontsizemultiplier="2.2" href="https:lutskrada.gov.ua">Луцька міська рада</a>                </div>                <img style="                    position: inherit;                     top: 50%;                     left: 50%;                    transform: translate(-50%, -50%);                    width: 25%;                    height: auto;                    " src="'.asset("img/logo_lutsk.png").'" alt="">                <div style="                    position: inherit;                    top: 75%;                    left: 5%;                    display: flex;                    align-items: center;                    width: 60%;                ">                    <img widthmultiplier="4.8" src="'.asset("img/Herb_Lutsk.svg").'" alt="" style="width: 27px;">                    <div style="padding-left: 1%; margin-left: 1%; border-width: 2px; border-left-style: solid; border-left-color: red;" borderwidthmultiplier="0.3">                        <p fontsizemultiplier="2" class="mb-0" style="font-size: 11.6px;">                            Міський голова                        </p>                        <p fontsizemultiplier="2" class="mb-0" style="font-size: 11.6px;">                            Пустовіт Григорій Олександрович                        </p>                    </div>                </div>                <script>                    let content = document.getElementById(slidesVariables[slidesVariables.length-1].id)                    let container = content.childNodes[5]                    let imageDiv = container.childNodes[3]                                        imageDiv.onclick = function() {                        container.childNodes[1].childNodes[1].setAttribute("href", "https://google.com.ua")                        let top = imageDiv.offsetTop;                        setInterval(() => {                            console.log(top)                            top = top - 10;                            imageDiv.style.top = top + "px";                        }, 40);                    }                </script>            '
        ],
        [
            'id' => 'cont1',
            'name' => 'Продовження 1',
...
@section('content')
    ...
    <div class="presentation-and-menu-container">
        <div class="col-2 shadow menu-container">
            <nav id="presentation">
                <nav class="nav nav-pills flex-column">
                    @foreach($presentation as $slide)

                        @if($loop->first)
                            <a class="nav-link" href="#{{ $slide['id'] }}">{{ $slide['name'] }}</a>
                            @continue
                        @endif
